...
class appDevDebugProjectContainerUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($pathinfo)
    {
        ...
        if ($pathinfo === '/utilizadoresx') {
            return array (  '_controller' => 'AppBundle\\Controller\\DefaultController::utilizadoresAction',  '_route' => 'utilizadores_s',);
        }
        ...
        if (0 === strpos($pathinfo, '/utilizadores')) {
            // utilizadores
            if (rtrim($pathinfo, '/') === '/utilizadores') {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
                    goto not_utilizadores;
                }

                if (substr($pathinfo, -1) !== '/') {
                    return $this->redirect($pathinfo.'/', 'utilizadores');
                }

                return array (  '_controller' => 'AppBundle\\Controller\\MUS\\UsuarioController::indexAction',  '_route' => 'utilizadores',);
            }
            not_utilizadores:

            // usuario_new
            if ($pathinfo === '/utilizadores/new') {
                if (!in_array($this->context->getMethod(), array('GET', 'POST', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'POST', 'HEAD'));
            ...
            // usuario_show
            if (preg_match('#^/utilizadores/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
            ...
            // usuario_edit
            if (preg_match('#^/utilizadores/(?P<id>[^/]++)/edit$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'POST', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'POST', 'HEAD'));
